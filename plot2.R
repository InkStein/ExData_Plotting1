...
##############
    ...
    png(filename = "plot2.png", height = 480, width = 480, units = "px") # opens a png device to encapsulate the plots so it displays correctly
    par(mar = c(6,6,3,3))    # set margins to show labels with a bit of white space
    plot(power_data_subset$Date_and_Time, power_data_subset$Global_active_power, type = "l", ylab = "Global Active Power (kilowatts)", xlab = "")
